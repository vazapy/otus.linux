...
# Координаты (FilePos) пока не трогаем, они прилетят из файла бэкапа!
echo Configuring Master HostUser...
sudo docker exec db-slave mysql -u root -proot -e CHANGE MASTER TO MASTER_HOST='$MASTER_HOST', MASTER_USER='$REPL_USER', MASTER_PASSWORD='$REPL_PASSWORD';
